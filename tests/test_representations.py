...
class TestSeries(unittest.TestCase):
    ...
    def test_frequency_series_methods(self):
        ...
        self.assertTrue(np.array_equal(result.entries, np.angle(self.freq_entries)))
...
class TestPhasor(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.phasor_sequence = Phasor.make(self.frequencies, self.amplitudes, self.phases)
    ...
    def test_cplx_to_phasor(self):
        ...
        amplitudes, phases = Phasor.cplx_to_phasor(complex_numbers)
    ...
    def test_reim_to_phasor(self):
        ...
        expected_amplitudes = np.array([1.0, 1.0, 1.0])
        expected_phases = np.array([0.0, np.pi/2, np.pi])
        amplitudes, phases = Phasor.reim_to_phasor(real_parts, imag_parts)
        assert_array_almost_equal(amplitudes, expected_amplitudes)
        assert_array_almost_equal(phases, expected_phases)
