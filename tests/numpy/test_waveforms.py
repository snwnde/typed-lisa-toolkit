...
def _linear_interpolator(x, y):
    ...
    def _interp(x_new):
        return np.interp(np.asarray(x_new, dtype=np.float64), x_arr, y_arr)
...
class TestHarmonicWaveformNumpy:
    def test_constructor_normalizes_tuple_mode_keys(self):
        wf = build_harmonic_waveform_constructor(np)

        assert tuple(wf.keys()) == (
            modes.Harmonic.cast((2, 2)),
            modes.Harmonic.cast((3, 3)),
        )
    ...
    def test_domain_and_pick(self):
        ...
        picked = wf.pick(case["expected"]["mode_22"])
        assert tuple(picked.keys()) == (case["expected"]["mode_22"],)
        npt.assert_allclose(
            np.asarray(picked[case["expected"]["mode_22"]].entries),
            np.asarray(case["expected"]["wf_22"].entries),
        )
...
class TestHarmonicProjectedWaveformNumpy:
    ...
    def test_mode_channels_ops_mapping_and_inplace(self):
        case = build_harmonic_projected_frequency_waveform(np)
        left = case["actual"]["wf"]
        right = build_harmonic_projected_waveform_constructor(np)
        kernel = np.asarray(left.get_kernel())

        combined = left + right
        npt.assert_allclose(np.asarray(combined.get_kernel()), kernel + kernel)

        left_copy = build_harmonic_projected_waveform_constructor(np)
        left_copy += right
...
class TestWaveformConstructorsNumpy:
    ...
    def test_projected_waveform_constructor(self):
        resp = build_projected_waveform_from_mapping(
            build_harmonic_projected_frequency_waveform(np)["expected"]["resp_22_map"],
        )
    ...
    def test_harmonic_projected_waveform_constructor(self):
        wf = build_harmonic_projected_waveform_constructor(np)
    ...
    def test_homogeneous_harmonic_projected_waveform_constructor(self):
        wf = build_homogeneous_harmonic_projected_waveform_constructor(np)
