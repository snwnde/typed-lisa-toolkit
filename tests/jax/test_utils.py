...
class TestUtilsJax(unittest.TestCase):
    ...
    def test_extend_to_1d(self):
        ...
        assert (np.asarray(extended[..., 3:8]) == np.asarray(entries)).all()
        assert (np.asarray(extended[..., :3]) == 0).all()
        assert (np.asarray(extended[..., 8:]) == 0).all()
