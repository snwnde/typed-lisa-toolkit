...
class TestTaperingJax(unittest.TestCase):
    def test_ldc_window_basic(self):
        ...
        assert np.asarray(win)[0] == pytest.approx(0, abs=1e-2)
        assert np.asarray(win)[-1] == pytest.approx(0, abs=1e-2)
        assert (np.asarray(win)[20:80] > 0.9).all()

    def test_planck_window_basic(self):
        grid = jnp.linspace(0, 100, 100)
        win = tapering.planck_window(left_margin=10, right_margin=10)(grid)
        assert np.asarray(win)[0] == 0
        assert np.asarray(win)[-1] == 0
        assert (np.asarray(win)[20:80] > 0.9).all()
    ...
    def test_get_tapering_func_str(self):
        ...
        win = taper(grid)
        assert np.asarray(win).shape == np.asarray(grid).shape
        assert (np.asarray(win) >= 0).all()
        assert (np.asarray(win) <= 1).all()
